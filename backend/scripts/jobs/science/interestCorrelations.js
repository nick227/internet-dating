...
export const scienceInterestCorrelationsJob = {
    ...
    run: async () => {
        ...
        for (let i = 0; i < interests.length; i += BATCH_SIZE) {
            ...
            for (const interestA of batch) {
                ...
                if (usersWithA.length < MIN_SHARED_USERS)
                    continue;
        ...
        `;
                for (const coOccur of coOccurrences) {
                    ...
                    if (jaccardScore < MIN_CORRELATION_SCORE)
                        continue;
                    // Calculate average match score for users sharing both interests
                    const avgMatchScoreResult = await prisma.$queryRaw `
