        ...
        for (const interestId of dirtyInterestIds) {
            try {
                ...
                for (const row of coInterestRows) {
                    pairCounts.set(row.interestId, (pairCounts.get(row.interestId) ?? 0) + 1);
                }
                ...
                const userCountByInterest = new Map();
                ...
                for (const [otherInterestId, pairCount] of pairCounts) {
                    if (pairCount < MIN_PAIR_COUNT)
                        continue;
                    const interestAId = interestId < otherInterestId ? interestId : otherInterestId;
                    const interestBId = interestId < otherInterestId ? otherInterestId : interestId;
                    // Get actual user counts for both interests
                    const otherInterestCount = userCountByInterest.get(otherInterestId) ?? 0;
                    if (otherInterestCount === 0)
                        continue; // Skip if no users have this interest
                    ...
                    ops.push(prisma.interestRelationship.upsert({
                        where: {
                ...
                console.error(`[interest-relationships] failed for interest ${interestId.toString()}`, err);
            }
        }
        console.log(`[interest-relationships] Processed ${processedInterests} interests, touched ${relationshipsTouched} relationships`);
    }
};
